        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="glass-card p-6">
            ...
            <h2 className="font-heading text-lg font-semibold text-foreground">
                {selectedDate === today ? "Today's Entry" : `Entry for ${new Date(selectedDate).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}`}
              </h2>
              <input
                type="date"
                value={selectedDate}
                      ...
                      {SYMPTOMS.map(s => (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-full text-xs border-none cursor-pointer transition-all ${
                            symptoms.includes(s.value) ? 'bg-primary text-primary-foreground' : 'bg-primary/10 text-foreground hover:bg-primary/20'
                          }`}
                        >
                          {s.emoji} {s.label}
                        </button>
